...
function renderBuyersList(type) {
  ...
  tbody.innerHTML = filteredBuyers.map(b => {
    ...
    const deleteBtn = activeRole === "HOST"
      ? `<button class="btn-delete" style="padding: 4px 8px; font-size: 0.8rem;" onclick="deleteSaleEntry('${encodedType}', ${b.originalIndex})">Delete</button>`
      : `<span style="color:#aaa; font-style:italic;">Read-Only</span>`;

    const shareBtn = `<button style="background-color: #25D366; color: white; border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 0.8rem; margin-right: 4px;" onclick="shareViaWhatsApp('${encodedType}', ${b.originalIndex})">WhatsApp</button>`;
    const printBtn = `<button style="background-color: #4b5563; color: white; border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 0.8rem; margin-right: 4px;" onclick="printReceipt('${encodedType}', ${b.originalIndex})">Print</button>`;
    ...
    return `
      <tr>
        ...
        <td>${b.mobile}</td>
        <td>${b.village}</td>
        <td>${b.qty} Bag(s) (₹${totalCost})</td>
        <td>${shareBtn}${printBtn}${deleteBtn}</td>
      </tr>
    `;
  }).join("");
}
  ...
  reader.onload = function(e) {
    try {
      ...
      if (parsedData["24%"] && parsedData["22%"] && parsedData["20%"]) {
        bagData = parsedData;
        saveDataToStorage();
        alert("Inventory and Sales data restored successfully!");
        location.reload();
      } else {
